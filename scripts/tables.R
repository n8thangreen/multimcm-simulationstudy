...
combined_cf_tab <- 
  merge(cf_tab_sep, cf_tab_hier, by = "Scenario", suffixes = c(".sep", ".hier")) |> 
  mutate(across(where(is.numeric), ~ round(., 2)))

combined_cf_tab <- 
  combined_tab[, order(names(combined_tab))] |> 
  select(Scenario, everything())

write.csv(combined_cf_tab, file = glue::glue("output_data/cf_table.csv"), row.names = FALSE)

combined_rmst_tab <- 
  merge(rmst_tab_sep, rmst_tab_hier, by = "Scenario", suffixes = c(".sep", ".hier")) |> 
  mutate(across(where(is.numeric), ~ round(., 2)))
