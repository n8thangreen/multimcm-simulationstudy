...
for (i in 1:nTx) {
 ...
 // restricted mean survival time
for (i in 1:nTx) {
  rmst_4[i] = cf_4[i]*t_max + (1 - cf_4[i])*rmst_weibull(shape_4, mean_4, t_max);
  median_4[i] = median_surv_cf_weibull(shape_4, mean_4, cf_4[i]);
  //median_4[i] = median_surv_weibull(shape_4, mean_4);  // uncured
}

 // restricted mean survival time
for (i in 1:nTx) {
  rmst_5[i] = cf_5[i]*t_max + (1 - cf_5[i])*rmst_weibull(shape_5, mean_5, t_max);
  median_5[i] = median_surv_cf_weibull(shape_5, mean_5, cf_5[i]);
  //median_5[i] = median_surv_weibull(shape_5, mean_5);  // uncured
}

 // restricted mean survival time
for (i in 1:nTx) {
  rmst_6[i] = cf_6[i]*t_max + (1 - cf_6[i])*rmst_weibull(shape_6, mean_6, t_max);
  median_6[i] = median_surv_cf_weibull(shape_6, mean_6, cf_6[i]);
  //median_6[i] = median_surv_weibull(shape_6, mean_6);  // uncured
}

 // restricted mean survival time
for (i in 1:nTx) {
  rmst_7[i] = cf_7[i]*t_max + (1 - cf_7[i])*rmst_weibull(shape_7, mean_7, t_max);
  median_7[i] = median_surv_cf_weibull(shape_7, mean_7, cf_7[i]);
  //median_7[i] = median_surv_weibull(shape_7, mean_7);  // uncured
}

 // restricted mean survival time
for (i in 1:nTx) {
  rmst_8[i] = cf_8[i]*t_max + (1 - cf_8[i])*rmst_weibull(shape_8, mean_8, t_max);
  median_8[i] = median_surv_cf_weibull(shape_8, mean_8, cf_8[i]);
  //median_8[i] = median_surv_weibull(shape_8, mean_8);  // uncured
}

 // restricted mean survival time
for (i in 1:nTx) {
  rmst_9[i] = cf_9[i]*t_max + (1 - cf_9[i])*rmst_weibull(shape_9, mean_9, t_max);
  median_9[i] = median_surv_cf_weibull(shape_9, mean_9, cf_9[i]);
  //median_9[i] = median_surv_weibull(shape_9, mean_9);  // uncured
}

 // restricted mean survival time
for (i in 1:nTx) {
  rmst_10[i] = cf_10[i]*t_max + (1 - cf_10[i])*rmst_weibull(shape_10, mean_10, t_max);
  median_10[i] = median_surv_cf_weibull(shape_10, mean_10, cf_10[i]);
  //median_10[i] = median_surv_weibull(shape_10, mean_10);  // uncured
}
// prior mean checks
